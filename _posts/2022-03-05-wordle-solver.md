...
The function `sort_by_freq` is used to sorted the wordlist based on a score of the words letter frequency. First, the function loops over each letter in every word in the wordlist. It then adds up the frequency of each letter in the word to give it a frequency score and stores it in `score_list`. Once each word has a score, the wordlist and score list are zipped and sorted by the score. `word_list` is then sorted and the function returns the `zipped_list`. 
...
Number of succsess: 2298
Average guesses for successes: 3.258485639686684
Fails: 17
Time: 39.430559396743774
```
...
class WordleSolve:
    ...
    # If we know a letter is or isn't in a word, adjust thr word list apropriatly
    def letter_modify(self, letter, remove):
        remove_list = []
        for word in self.word_list:
            if remove:
